...
bool glimmer::InitModContextTask::Run(ISystemBucket *systemBucket) {
    ...
    if (langsResources == nullptr) {
        LogCat::e(std::source_location::current(), "langsResources is nullptr");
        return false;
    }
    auto modContext = std::make_unique<ModContext>();
    modContext->Init(virtualFileSystem, langsResources);
    systemBucket->SetModContext(std::move(modContext));
    return true;
}
