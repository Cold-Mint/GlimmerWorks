...
bool glimmer::PackSignatureVerifier::ProcessPublicKeyFile(const std::filesystem::path &path) const {
    ...
    if (publicKeyStreamUniquePtr == nullptr) {
        LogCat::w(LogLabel::DEFAULT, std::source_location::current(), "data_pack_public_key_read_failed",
    ...
    if (pubStream.gcount() == 32) {
        ...
        LogCat::d(LogLabel::DEFAULT, "data_pack_public_key_loaded", "Loaded public key file: {}", path.string());
    }
    return true;
}
...
bool glimmer::PackSignatureVerifier::ProcessSignatureFile(const std::filesystem::path &path) const {
    ...
    if (signStreamUniquePtr == nullptr) {
        LogCat::w(LogLabel::DEFAULT, std::source_location::current(), "data_pack_signature_read_failed",
    ...
    if (sigStream.gcount() == 64) {
        ...
        LogCat::d(LogLabel::DEFAULT, "data_pack_signature_loaded", "Loaded signature file: {}", path.string());
    }
    return true;
}
...
glimmer::PackVerifyState glimmer::PackSignatureVerifier::VerifySignature(const bool findPublicKey,
    ...
    if (!findPublicKey || !findSignature) {
        LogCat::d(LogLabel::DEFAULT, "data_pack_signature_missing",
                  "Signature verification skipped: missing public key or signature");
        return VerifiedFailed;
    }
    if (crypto_ed25519_check(signature.data(), publicKey.data(),
                             allHashData.data(), allHashData.size()) == 0) {
        LogCat::i(LogLabel::DEFAULT, "data_pack_signature_verified", "Data pack signature verified successfully");
        return VerifiedSuccess;
    }
    LogCat::w(LogLabel::DEFAULT, std::source_location::current(), "data_pack_signature_invalid",
