...
SDL_Color glimmer::ColorUtils::LerpColor(const SDL_Color from, const SDL_Color to, const float percent) {
    ...
    const Uint8 r = static_cast<Uint8>(from.r + (to.r - from.r) * blend_ratio);
    const Uint8 g = static_cast<Uint8>(from.g + (to.g - from.g) * blend_ratio);
    const Uint8 b = static_cast<Uint8>(from.b + (to.b - from.b) * blend_ratio);
    const Uint8 a = static_cast<Uint8>(from.a + (to.a - from.a) * blend_ratio);
    return {r, g, b, a};
}
